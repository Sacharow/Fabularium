...
export default function QuestNew() {
  ...
  const saveQuest = () => {
    ...
    fetch(`http://localhost:3000/api/campaigns/${campaignId}/missions`, {
      ...
      .then((created) => {
        if (campaignId)
          navigate(`/InCampaign/${campaignId}/Quests/${created.id}`);
        else navigate(-1);
      })
      .catch((e) => {
    ...
    fetch(`http://localhost:3000/api/campaigns/${campaignId}/locations`, {
      ...
      .then((data) => {
        const locs = Array.isArray(data)
          ? data.map((l: any) => ({ id: l.id, name: l.name ?? String(l.id) }))
          : [];
        setAvailableLocations(locs);
      })
      .catch((e) => console.error("Failed to load locations", e));

    fetch(`http://localhost:3000/api/campaigns/${campaignId}/npcs`, {
      credentials: "include",
    })
      .then(async (res) => {
      ...
      .then((data) => {
        const npcs = Array.isArray(data)
          ? data.map((n: any) => ({ id: n.id, name: n.name ?? String(n.id) }))
          : [];
                ...
                <ul className="space-y-2">
                  ...
                  <li>
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 rounded ${selectedView === "locations" ? "bg-orange-700 text-white" : "text-orange-300 hover:bg-orange-800"}`}
                    >
                      Location
                    </button>
                  </li>
                  <li>
                    <button
                      onClick={() => setSelectedView("npcs")}
                      className={`w-full text-left px-3 py-2 rounded ${selectedView === "npcs" ? "bg-orange-700 text-white" : "text-orange-300 hover:bg-orange-800"}`}
                    >
                      Npc
                    </button>
                  </li>
                  <li>
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 rounded ${selectedView === "rewards" ? "bg-orange-700 text-white" : "text-orange-300 hover:bg-orange-800"}`}
                    >
                      Rewards
                    </button>
                  </li>
                  ...
                  <div className={inputGameplayInformation}>
                    ...
                    {locationsIds.map((locId, index) => {
                      ...
                      return (
                        <div
                          key={index}
                          className="flex justify-between items-center border-b-2 border-l-2 border-r-2 border-orange-700 p-1"
                        >
                          <p>{name}</p>
                          <button
