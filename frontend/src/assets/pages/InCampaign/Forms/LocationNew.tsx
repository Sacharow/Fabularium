...
export default function LocationNew() {
    ...
    if (name.trim() === "") {
      ...
      fetch(`http://localhost:3000/api/campaigns/${campaignId}/locations`, {
        ...
        .then(async (created) => {
          ...
          try {
            if (
              Array.isArray(selectedQuestIds) &&
              selectedQuestIds.length > 0
            ) {
              await Promise.all(
                selectedQuestIds.map((mid) =>
                  fetch(
                    `http://localhost:3000/api/campaigns/${campaignId}/missions/${mid}`,
                    {
                      method: "PUT",
                      credentials: "include",
                      headers: { "Content-Type": "application/json" },
          ...
          if (campaignId)
            navigate(`/InCampaign/${campaignId}/Locations/${created.id}`);
          else navigate(-1);
        })
        .catch((e) => {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:3000/api/campaigns/${campaignId}`, {
      ...
      .then((data) => {
        const qList = Array.isArray(data.missions)
    ...
    fetch(`http://localhost:3000/api/campaigns/${campaignId}/npcs`, {
      ...
      .then((data) => {
        const npcNames = Array.isArray(data)
          ? data.map((n: any) => n.name ?? String(n.id))
          : [];
                ...
                <ul className="space-y-2">
                  ...
                  <li>
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 rounded ${selectedView === "quests" ? "bg-orange-700 text-white" : "text-orange-300 hover:bg-orange-800"}`}
                    >
                      Quests
                    </button>
                  </li>
