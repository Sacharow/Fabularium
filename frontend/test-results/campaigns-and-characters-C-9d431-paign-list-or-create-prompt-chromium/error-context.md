...
- Name: campaigns-and-characters.spec.ts >> Campaign Management >> should display campaign list or create prompt
- Location: e2e\campaigns-and-characters.spec.ts:13:3

# Error details

```
Error: locator.isVisible: Error: strict mode violation: locator('button:has-text("Create"), button:has-text("New")') resolved to 2 elements:
    1) <button disabled type="button" aria-disabled="true" class="p-2 my-2 w-full border-2 border-gray-neutral bg-dark text-gray-neutral opacity-60 cursor-not-allowed">…</button> aka getByRole('button', { name: 'CREATE NEW' })
    2) <button disabled type="button" aria-disabled="true" class="p-2 my-2 w-full border-2 border-gray-neutral bg-dark text-gray-neutral opacity-60 cursor-not-allowed">…</button> aka locator('button').nth(2)

Call log:
    - checking visibility of locator('button:has-text("Create"), button:has-text("New")')
