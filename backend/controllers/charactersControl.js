...
const createCharacter = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Failed to create character", error: String(err) });
  }
};
...
const getCharacterById = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Failed to fetch character", error: String(err) });
  }
};
...
const updateCharacter = async (req, res) => {
  try {
    ...
    console.log("🔄 Update character request:", {
      characterId: id,
      body: req.body,
    });
    ...
    console.log("✅ Validation passed, updating character...");
    const { error, character } = await updateOwnedCharacter(
      id,
      user.id,
      parsed.data,
    );
    ...
    return res
      .status(500)
      .json({ message: "Failed to update character", error: String(err) });
  }
};
...
const deleteCharacter = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Failed to delete character", error: String(err) });
  }
};
